...
import com.myxianwen.xvideoviewext.Settings;

import java.util.Map;


public class XVideoViewExt extends SurfaceView
        implements MediaController.MediaPlayerControl, OrientationDetector.OrientationChangeListener{
    private String TAG = "XVideoViewExt";
    ...
    public XVideoViewExt(Context context) {
        this(context,null);
    }

    public XVideoViewExt(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public XVideoViewExt(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;
        TypedArray a = mContext.obtainStyledAttributes(attrs, R.styleable.XVideoView,0,0);
        mFitXY = a.getBoolean(R.styleable.XVideoView_uvv_fitXY, false);
        mAutoRotation = a.getBoolean(R.styleable.XVideoView_uvv_autoRotation, false);
        a.recycle();
        initVideoView();
    }
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(XVideoViewExt.class.getName());
    }
    ...
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            info.setClassName(XVideoViewExt.class.getName());
        }
    }
    ...
    private void openVideo() {
        ...
        if(mSettings.getPlayer() == Settings.X_PLAYER__QPMediaPlayer) {
            mQuPlayer = new QuPlayerExt();
            mQuPlayer.setErrorListener(_ErrorListener);
            mQuPlayer.setInfoListener(_InfoListener);
            mQuPlayer.setSurface(mSurfaceHolder.getSurface());

            mCurrentState = STATE_PREPARING;
            attachMediaController();

            DataSpec spec = new DataSpec(mUri.toString(), DataSpec.MEDIA_TYPE_STREAM);
    ...
    private void attachMediaController() {
        if (mQuPlayer != null && mMediaController != null) {
    ...
    SurfaceHolder.Callback mSHCallback = new SurfaceHolder.Callback()
    {
                                   ...
                                   int w, int h)
        {
            ...
            if (mQuPlayer != null && isValidState && hasValidSize) {
    ...
    private void enableOrientationDetect() {
        if (mAutoRotation && mOrientationDetector == null) {
            ...
            mOrientationDetector.setOrientationChangeListener(XVideoViewExt.this);
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event)
    {
        ...
        if (isInPlaybackState() && isKeyCodeSupported && mMediaController != null) {
            if (keyCode == KeyEvent.KEYCODE_HEADSETHOOK ||
                ...
                if (mQuPlayer != null && mQuPlayer.isPlaying()) {
                    pause();
    ...
    public void start() {
        ...
        if (isInPlaybackState()) {
            ...
            if (this.videoViewCallback != null) {
                this.videoViewCallback.onStart(mQuPlayer);
            }
        }
    ...
    public void pause() {
        if (isInPlaybackState()) {
            if (mQuPlayer.isPlaying()) {
                ...
                if (this.videoViewCallback != null) {
                    this.videoViewCallback.onPause(mQuPlayer);
                }
            }
        }
    ...
    public int getDuration() {
        if (isInPlaybackState()) {
            return (int)mQuPlayer.getDuration();
        }
    ...
    public int getCurrentPosition() {
        if (isInPlaybackState()) {
            return (int)mQuPlayer.getCurrentPosition();
        }
        return 0;
    }
    ...
    public boolean isPlaying() {
        return isInPlaybackState() && mQuPlayer.isPlaying();
    }
...
*/
    ...
    public interface VideoViewCallback {
        ...
        void onPause(final QuPlayerExt mediaPlayer);
        void onStart(final QuPlayerExt mediaPlayer);
        void onBufferingStart(final QuPlayerExt mediaPlayer);
        void onBufferingEnd(final QuPlayerExt mediaPlayer);
    }
